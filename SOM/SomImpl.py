...
from numpy import (array, unravel_index, nditer, linalg, random, subtract,
                   power, exp, pi, zeros, arange, outer, meshgrid, dot, multiply, divide, std, mean,corrcoef,
...
class MiniSom(object):
    ...
    def _activate(self, x):
        ...
        while not it.finished:
            # || x - w ||
            dist = correlation(x, self._weights[it.multi_index])
            ...
            for ii in range(it.multi_index[0]-1, it.multi_index[0]+2):
                for jj in range(it.multi_index[1]-1, it.multi_index[1]+2):
                    if (ii >= 0 and ii < self._weights.shape[0] and
                        ...
                        um[it.multi_index] += correlation(w_1,w_2)#fast_norm(w_1-w_2)
            it.iternext()
        um = um/um.max()
        return um
